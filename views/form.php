...
<body class="bg-gray-200 text-white">
 ...
 <?php if ($questionary) : ?>
  <div class="container mx-auto px-4">
    <h1 class="text-2xl font-bold my-4 text-gray-800"><?= $questionary["questionary"]->name ?></h1>
    <p class="text-lg text-gray-600"><?= $questionary["questionary"]->description ?></p>
    ...
    <div class="questionary" data-questionary-id="<?= $questionary["questionary"]->id ?>">
      <form method="post" action="../controllers/general_form.php" class="bg-violet-200 shadow-md rounded px-8 pt-6 pb-8 mb-4"> 
        <div id="questions-container">
          <?php foreach ($questionary["questions"] as $question) : ?>
            <div class="question mb-4" style="display: none;">
              <h3 class="text-2xl font-bold text-gray-800"><?= $question["value"] ?></h3>
              <ul class="list-none pl-5">
                <?php foreach ($question["options"] as $option) : ?>
                 <li class="my-2">
                   <input type="radio" name="answer_<?= $question["id"] ?>" value="<?= $option["id"] ?>" class="mr-2" required>
                   <label class="text-lg text-gray-700"><?= $option["value"] ?></label>
                 </li>
                <?php endforeach; ?>
              </ul>
            </div>
        ...
        <button type="button" id="next-button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800  focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 " >Siguiente</button>
